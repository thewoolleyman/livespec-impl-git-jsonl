# tests/livespec_orchestrator_git_jsonl/commands/

Tests for the thin-transport command modules under
`.claude-plugin/scripts/livespec_orchestrator_git_jsonl/commands/`. One
`test_<name>.py` per module:

- `test_next.py` — the ranker. Asserts `rank_candidates` /
  `build_envelope` produce the correct ripeness ordering (`rank`, then
  `id` as the tiebreak — the sole ordering authority since `priority`
  was removed in v013), the uniform advisory `urgency`, the
  `{candidates[], pagination}` envelope shape, `--limit`/`--offset`
  slicing, the empty-list no-work signal, and `depends_on` readiness
  gating (candidates with an OPEN dependency are absent from the ranked
  list).
- `test_list_work_items.py` — listing, filtering, and the
  `--json` vs human output contracts.
