...
import pytest
from livespec_orchestrator_git_jsonl.checks.no_raw_store_read import main
...
def _fixture_root(*, tmp_path: Path) -> Path:
    ...
    _write_module(
        root=root,
        relative=".claude-plugin/scripts/livespec_orchestrator_git_jsonl/store.py",
        source=(
            "from pathlib import Path\n"
            "\n"
            "\n"
...
def test_main_passes_on_conforming_tree(
    ...
    _write_module(
        root=root,
        relative=".claude-plugin/scripts/livespec_orchestrator_git_jsonl/clean.py",
        source=(
            "from pathlib import Path\n"
            "\n"
            "\n"
...
def test_main_fails_on_literal_store_basename_open(
    ...
    _write_module(
        root=root,
        relative=".claude-plugin/scripts/livespec_orchestrator_git_jsonl/offender.py",
        source=(
            "from pathlib import Path\n"
            "\n"
            "\n"
...
def test_main_fails_on_declared_path_attribute_open(
    ...
    _write_module(
        root=root,
        relative=".claude-plugin/scripts/livespec_orchestrator_git_jsonl/attr_offender.py",
        source=(
...
def test_main_ignores_non_open_calls_and_unrelated_literals(
    ...
    _write_module(
        root=root,
        relative=".claude-plugin/scripts/livespec_orchestrator_git_jsonl/benign.py",
        source=(
