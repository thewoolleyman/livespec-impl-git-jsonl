# tests/livespec_orchestrator_git_jsonl/

Tests for the `livespec_orchestrator_git_jsonl` package under
`.claude-plugin/scripts/livespec_orchestrator_git_jsonl/`. Mirrors the
...
- Build minimal JSONL fixtures in `tmp_path`; do NOT read or write
  the repo's real work-items file. Append-only behavior is
