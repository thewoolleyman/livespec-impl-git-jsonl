...
- `conftest.py` provides the `wrapper_runner` fixture: it
  ...
  stubbed `livespec_orchestrator_git_jsonl.<module>.main` (so the wrapper's
...
- `test_<cmd>.py` — one per wrapper (`detect_impl_gaps`,
  `list_work_items`, `next`, `migrate_beads`). Each
...
`tests/livespec_orchestrator_git_jsonl/`). Do NOT import the real
