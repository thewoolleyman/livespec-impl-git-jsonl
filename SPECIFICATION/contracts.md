...
`livespec-core/SPECIFICATION/contracts.md`; nothing here overrides
upstream.
...
doctor's cross-boundary invariants in `livespec-core` invoke skills
through this namespace prefix per
`livespec-core/SPECIFICATION/contracts.md` §"Cross-plugin
...
consent, hand off to `/livespec-core:propose-change` via the
...
3. Performs the disposition action:
   - `spec-bound` — hand off to `/livespec-core:propose-change`
...
in `livespec-core/SPECIFICATION/contracts.md` §"Wrapper CLI
...
invariant in `livespec-core` reads materialized views; readers
...
Per `livespec-core/SPECIFICATION/contracts.md` §"Spec Reader
...
Per `livespec-core/SPECIFICATION/contracts.md` §"Persistent Agent
...
- Topic files MAY grow over time; pruning is the user's call
  ...
  invariant in `livespec-core` does NOT apply to
...
This realization mirrors `livespec-core`'s own v058-era
`.ai/<topic>.md` convention (now graduated to a first-class
upstream contract slot).

## `compat` block

Per `livespec-core/SPECIFICATION/contracts.md` §"Cross-repo
...
`livespec_core` is a semver range matching every `livespec-core`
release this plugin's pinned version is known to be compatible
with. `pinned` is the SPECIFIC `livespec-core` release tag the
