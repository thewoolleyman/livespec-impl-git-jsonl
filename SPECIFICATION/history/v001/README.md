...
- [`constraints.md`](constraints.md) — architecture-level rules
  ...
  references upstream `livespec-core` non-functional requirements;
...
- [`proposed_changes/`](proposed_changes/) — pending propose-change
  files awaiting `/livespec-core:revise`.
...
FIXED by `livespec-core`. This `SPECIFICATION/` describes the
