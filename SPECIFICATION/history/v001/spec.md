...
the JSONL-backed implementation plugin for `livespec-core`. The plugin
dogfoods `livespec-core` — this `SPECIFICATION/` tree evolves through
`/livespec-core:seed` / `propose-change` / `revise` / `doctor` /
...
implementation-plugin contract that `livespec-core` publishes in
`livespec-core/SPECIFICATION/contracts.md` §"Implementation-plugin
...
`git grep`. That property is the principal reason it is `livespec-core`'s
...
store realization slot — is FIXED by `livespec-core`'s published
contract. This `SPECIFICATION/` MUST NOT re-state `livespec-core`'s
contract; it MUST concretize the contract for the JSONL substrate and
point upstream for anything else.

When `livespec-core`'s contract changes, this plugin's `compat` block
pin moves forward in a discrete bump-pin PR (per `livespec-core`'s
...
pinned `livespec-core` reference is recorded in `.copier-answers.yml`
(`livespec_core_release_tag`) and in `.livespec.jsonc`'s
`livespec-impl-plaintext.compat` block.

## Terminology

This spec adopts every term defined in
`livespec-core/SPECIFICATION/spec.md` §"Terminology" verbatim
...
- Not a re-statement of `livespec-core`'s contract. When in doubt,
  defer to `livespec-core/SPECIFICATION/`.
...
- Not a substitute for the upstream invariant catalog. Doctor
  ...
  `livespec-core/SPECIFICATION/contracts.md` §"Doctor cross-boundary
...
This `SPECIFICATION/` is governed by `livespec-core`. Changes land
through the standard livespec lifecycle:

- Propose: `/livespec-core:propose-change --spec-target SPECIFICATION/`
  drops a file under `proposed_changes/`.
- Critique: `/livespec-core:critique --spec-target SPECIFICATION/`
  surfaces issues before they ratify.
- Revise: `/livespec-core:revise --spec-target SPECIFICATION/`
  accepts, modifies, or rejects each pending proposal and snapshots
  a new `history/vNNN/`.
- Doctor: `/livespec-core:doctor --spec-target SPECIFICATION/` runs
  static + LLM-driven invariants.
- Prune: `/livespec-core:prune-history --spec-target SPECIFICATION/`
