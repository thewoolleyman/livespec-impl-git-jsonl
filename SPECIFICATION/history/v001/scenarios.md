...
`livespec-core/SPECIFICATION/`. These are not test cases (those
live under `tests/`); they are reader-facing journeys an agent
or contributor follows.

## Scenario 1 — Gap-tied fix cycle

A consumer project has a fresh `livespec-core` revision (vNNN+1)
...
3. For this memo, the user picks `spec-bound`. The skill hands
   off to `/livespec-core:propose-change` with the memo content
...
5. The next `/livespec-core:doctor` pass sees one fewer untriaged
...
The user invokes `/livespec-core:doctor` in a consumer project.
...
1. The driver invokes `/livespec-core:next --json` to get a
