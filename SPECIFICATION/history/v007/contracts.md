...
fifteen keys enumerated below are the canonical schema. The first
fourteen (`id` through `superseded_by`) are required-on-write AND
required-on-read; the fifteenth (`spec_commitment_hint`) is
required-on-write but optional-on-read (legacy records authored
before this field's introduction read back as `null` without firing
a schema violation). Additional keys beyond this fifteen are
forbidden — schema violations fire as doctor `fail` findings.
