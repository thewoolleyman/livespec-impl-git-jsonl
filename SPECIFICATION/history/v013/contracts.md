...
- `candidates[]` — array of candidate objects. `action` MUST be
  ...
  wrapper emits (e.g., `rank`, `origin`); the cross-plugin
...
seventeen keys enumerated below are the canonical schema. Fourteen
...
`spec_commitment_hint` and `supersedes` are required-on-write but
optional-on-read (legacy records authored before each field's
introduction read back as `null` without firing a schema violation).
Additional keys beyond this seventeen are forbidden — schema
